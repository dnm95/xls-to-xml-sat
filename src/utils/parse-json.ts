...
export function parseJSON(json: ExcelKeys[], catalog: boolean, balance: boolean, data: Data) {
  ...
  if (catalog) {
    ...
    json.forEach((obj: ExcelKeys) => {
      catalogXML = catalogXML + `<catalogocuentas:Ctas CodAgrup="${obj.bundlerCode}" NumCta="${obj.accountNumber}" Desc="${String(obj.description).replaceAll(/&/g, "&amp;")}" Nivel="${obj.level}" Natur="${obj.nature}" />\n`
    })
